...
class UserManager:
    ...
    def update_ai_avatar(self, user_id, ai_avatar_url):
        """
        更新用户的AI助理头像

        参数:
            user_id: 用户ID
            ai_avatar_url: AI助理头像URL

        返回:
            True表示成功，False表示失败
        """
        try:
            sql = "UPDATE users SET ai_avatar_url = %s WHERE id = %s"
            self.db.execute(sql, (ai_avatar_url, user_id))
            print(f"✅ 用户 {user_id} 的AI头像已更新: {ai_avatar_url}")
            return True
        except Exception as e:
            print(f"❌ 更新AI头像失败: {e}")
            return False

    def update_ai_assistant_name(self, user_id, ai_assistant_name):
        """
        更新用户的AI助理名字

        参数:
            user_id: 用户ID
            ai_assistant_name: AI助理名字

        返回:
            True表示成功，False表示失败
        """
        try:
            sql = "UPDATE users SET ai_assistant_name = %s WHERE id = %s"
            self.db.execute(sql, (ai_assistant_name, user_id))
            print(f"✅ 用户 {user_id} 的AI助理名字已更新: {ai_assistant_name}")
            return True
        except Exception as e:
