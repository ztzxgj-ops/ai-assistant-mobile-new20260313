...
class BackupHistoryManager:
    ...
    def get_recent_completed_tasks(self, hours=24):
        ...
        try:
            ...
            # 查询最近24小时内完成的任务（支持 work_tasks 和 work_plans 两个表）
            query = """
    ...
    def generate_git_notes(self):
        ...
        try:
            # ✨ 优先使用开发日志生成备注
            if self.dev_log:
                dev_notes = self.dev_log.generate_backup_notes()
                if dev_notes:
                    return dev_notes
            ...
            if commit_msgs:
                ...
                # 没有新提交，但有未提交的修改
                # ✨ 尝试从数据库中获取最近完成的任务来生成备注
                completed_tasks = self.get_recent_completed_tasks(hours=24)
                if completed_tasks:
                    # 如果有最近完成的任务，使用这些任务生成备注
                    if len(completed_tasks) == 1:
                        return f"✅ 完成: {completed_tasks[0]}"
                    else:
                        task_list = "; ".join(completed_tasks[:3])
                        if len(completed_tasks) > 3:
                            return f"✅ 完成多项任务: {task_list} 等"
                        return f"✅ 完成: {task_list}"

                # 如果没有完成的任务，尝试从对话记录中提取需求
                user_requests = self.get_recent_user_requests(hours=24)
                if user_requests:
                    # 如果有最近的需求，使用这些需求生成备注
                    if len(user_requests) == 1:
                        return f"📝 实现: {user_requests[0]}"
                    else:
                        request_list = "; ".join(user_requests[:2])
                        return f"📝 实现: {request_list}"

                # 如果都没有，提示用户手动填写功能描述
                file_count = len(uncommitted)
                return f"⚠️ 有 {file_count} 个文件修改，请点击'编辑备注'添加功能说明"
            else:
                return "无新提交"
    ...
    def add_backup_record(self, backup_dir, status='success', notes='', error_msg=''):
        ...
        # ✨ 如果备份成功，标记开发日志为已备份
        if status == 'success' and self.dev_log:
            try:
                count = self.dev_log.mark_all_as_backuped()
                if count > 0:
                    print(f"✅ 已标记 {count} 条开发日志为已备份")
            except Exception as e:
                print(f"标记开发日志失败: {e}")
