...
# 创建云服务器 bak 目录
ssh ${SERVER} "mkdir -p /var/www/ai-assistant/bak" 2>/dev/null

echo "正在上传..."
# 上传到 backups 目录
scp "${LOCAL_BAK_DIR}/${BACKUP_NAME}.tar.gz" ${SERVER}:${SERVER_BAK_DIR}/ 2>/dev/null
# 同时上传到 bak 目录
scp "${LOCAL_BAK_DIR}/${BACKUP_NAME}.tar.gz" ${SERVER}:/var/www/ai-assistant/bak/ 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ 上传成功${NC}"
    echo -e "${GREEN}📍 服务器路径 (backups): ${SERVER}:${SERVER_BAK_DIR}/${BACKUP_NAME}.tar.gz${NC}"
    echo -e "${GREEN}📍 服务器路径 (bak): ${SERVER}:/var/www/ai-assistant/bak/${BACKUP_NAME}.tar.gz${NC}"
else
