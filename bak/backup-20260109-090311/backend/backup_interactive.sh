...
echo "开始时间: $(date)"
echo "目标服务器: ${SERVER}"
echo "管理员密码: gyq3160GYQ3160"
echo ""
echo "⚠️  执行过程中会多次要求输入密码"
echo "    请复制粘贴: gyq3160GYQ3160"
echo ""
read -p "按回车键开始备份..."
...
CODE_COUNT=$(ls -1 ${BACKUP_DIR}/code/*.py 2>/dev/null | wc -l)
...
# 获取最新的备份文件名
LATEST_BACKUP=$(ssh ${USER}@${SERVER} "ls -t /tmp/ai-assistant-backup-*.tar.gz 2>/dev/null | head -1")
...
if [ $? -eq 0 ]; then
    ...
    # 清理服务器临时文件
    echo ""
    read -p "是否清理服务器临时文件？(y/n) " CLEAN_CHOICE

    if [ "$CLEAN_CHOICE" = "y" ] || [ "$CLEAN_CHOICE" = "Y" ]; then
        echo "清理中..."
        ssh ${USER}@${SERVER} "rm -rf /tmp/ai-assistant-backup-*"
        echo "✓ 服务器临时文件已清理"
    fi
    ...
    echo "⚠️  安全提醒"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "1. 备份包含数据库密码: gyq3160GYQ3160"
    echo "2. 备份包含API密钥"
    echo "3. 请将备份文件加密存储"
    echo "4. 建议修改对话中暴露的密码"
    echo ""
    ...
    echo "❌ 下载失败"
    echo "请手动下载:"
    echo "   scp ${USER}@${SERVER}:${LATEST_BACKUP} ./"
fi
