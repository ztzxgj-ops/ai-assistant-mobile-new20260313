...
mkdir -p "${TEMP_DIR}"/{backend,projects,dependencies,uploads,database,config,docs}
...
REMINDERS_PATH=~/.pub-cache/hosted/pub.dev/reminders-2.0.2
...
#### 1. lib/reminder.dart
**路径**: `~/.pub-cache/hosted/pub.dev/reminders-2.0.2/lib/reminder.dart`
...
**路径**: `~/.pub-cache/hosted/pub.dev/reminders-2.0.2/ios/Classes/Reminders.swift`
...
cp -r dependencies/reminders-2.0.2-modified \
  ~/.pub-cache/hosted/pub.dev/reminders-2.0.2

# 验证修改
cat ~/.pub-cache/hosted/pub.dev/reminders-2.0.2/lib/reminder.dart | grep recurrence
...
grep -n "String? recurrence" ~/.pub-cache/hosted/pub.dev/reminders-2.0.2/lib/reminder.dart

# 检查 daily case 是否存在
grep -n "case \"daily\"" ~/.pub-cache/hosted/pub.dev/reminders-2.0.2/ios/Classes/Reminders.swift
...
# 数据库备份
echo "正在备份 MySQL 数据库..."
ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "mysqldump -u root -pgyq3160GYQ3160 --no-data ai_assistant 2>/dev/null" > "${TEMP_DIR}/database/schema.sql" && echo -e "${GREEN}✓${NC} 数据库结构" || echo -e "${YELLOW}⚠️  数据库结构备份失败${NC}"

ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "mysqldump -u root -pgyq3160GYQ3160 ai_assistant 2>/dev/null" > "${TEMP_DIR}/database/full_backup.sql" && echo -e "${GREEN}✓${NC} 完整数据" || echo -e "${YELLOW}⚠️  数据库备份失败${NC}"

# 数据库统计
ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "mysql -u root -pgyq3160GYQ3160 ai_assistant -e \"
SELECT
    TABLE_NAME as 'Table',
    TABLE_ROWS as 'Rows',
...
echo ""
echo "正在备份配置文件..."
scp -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP}:/etc/nginx/sites-available/default "${TEMP_DIR}/config/nginx_default.conf" 2>/dev/null && echo -e "${GREEN}✓${NC} Nginx配置" || echo -e "${YELLOW}⚠️  Nginx配置未找到${NC}"

scp -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP}:/etc/supervisor/conf.d/ai-assistant.conf "${TEMP_DIR}/config/supervisor.conf" 2>/dev/null && echo -e "${GREEN}✓${NC} Supervisor配置" || echo -e "${YELLOW}⚠️  Supervisor配置未找到${NC}"

ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "supervisorctl status 2>/dev/null" > "${TEMP_DIR}/config/service_status.txt" && echo -e "${GREEN}✓${NC} 服务状态" || true

# 服务器代码备份
echo ""
echo "正在备份服务器代码..."
SERVER_CODE_BACKUP="server_code_${TIMESTAMP}.tar.gz"

# 在服务器上打包代码（排除uploads、bak、__pycache__）
ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "cd ${SERVER_PATH} && tar --exclude='uploads' --exclude='bak' --exclude='__pycache__' --exclude='*.pyc' -czf /tmp/${SERVER_CODE_BACKUP} *.py *.json *.md *.sh deploy/ 2>/dev/null" && echo -e "${GREEN}✓${NC} 服务器代码打包" || echo -e "${YELLOW}⚠️  服务器代码打包失败${NC}"

# 下载到本地
scp -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP}:/tmp/${SERVER_CODE_BACKUP} "${TEMP_DIR}/server_code/${SERVER_CODE_BACKUP}" 2>/dev/null && echo -e "${GREEN}✓${NC} 服务器代码下载" || echo -e "${YELLOW}⚠️  服务器代码下载失败${NC}"

# 清理服务器临时文件
ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "rm -f /tmp/${SERVER_CODE_BACKUP}" 2>/dev/null

SERVER_CODE_SIZE=$(du -sh ${TEMP_DIR}/server_code/ 2>/dev/null | cut -f1)
echo -e "${YELLOW}📊 服务器代码: ${SERVER_CODE_SIZE}${NC}"
...
mkdir -p ~/.pub-cache/hosted/pub.dev/
...
grep -n "String? recurrence" \
  ~/.pub-cache/hosted/pub.dev/reminders-2.0.2/lib/reminder.dart

# 检查 daily case
grep -n "case \"daily\"" \
  ~/.pub-cache/hosted/pub.dev/reminders-2.0.2/ios/Classes/Reminders.swift
...
mv "${TEMP_DIR}"/uploads "${TEMP_DIR}/${ARCHIVE_DIR_NAME}/" 2>/dev/null
mv "${TEMP_DIR}"/config "${TEMP_DIR}/${ARCHIVE_DIR_NAME}/" 2>/dev/null
mv "${TEMP_DIR}"/docs "${TEMP_DIR}/${ARCHIVE_DIR_NAME}/" 2>/dev/null
mv "${TEMP_DIR}"/server_code "${TEMP_DIR}/${ARCHIVE_DIR_NAME}/" 2>/dev/null

# ⚠️ 关键修复：在打包前修复所有文件权限，确保用户解压后可以看到所有文件
echo "修复文件权限，确保解压后可访问..."
find "${TEMP_DIR}/${ARCHIVE_DIR_NAME}" -type d -exec chmod 755 {} + 2>/dev/null || true
find "${TEMP_DIR}/${ARCHIVE_DIR_NAME}" -type f -exec chmod 644 {} + 2>/dev/null || true
...
1. ai-assistant-backup-${TIMESTAMP}.tar.gz
   ...
   - 用户上传文件 ($((${AVATARS_COUNT:-0} + ${IMAGES_COUNT:-0} + ${FILES_COUNT:-0})) 个文件)
...
4. 恢复用户上传文件：
   cp -r uploads/* /var/www/ai-assistant/uploads/
...
# 创建服务器备份目录
ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "mkdir -p ${SERVER_BAK_DIR}" 2>/dev/null
# 创建云服务器 bak 目录
ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "mkdir -p /var/www/ai-assistant/bak" 2>/dev/null
...
if [ -f "${BACKUP_DIR_NAME}.tar.gz" ]; then
    ...
    echo "正在上传 (可能需要几分钟)..."
    # 上传到 backups 目录
    scp -o StrictHostKeyChecking=no "${BACKUP_DIR_NAME}.tar.gz" ${SERVER_USER}@${SERVER_IP}:${SERVER_BAK_DIR}/ 2>/dev/null

    # 同时上传到 bak 目录
    scp -o StrictHostKeyChecking=no "${BACKUP_DIR_NAME}.tar.gz" ${SERVER_USER}@${SERVER_IP}:/var/www/ai-assistant/bak/ 2>/dev/null

    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ 上传成功${NC}"
        echo -e "${GREEN}📍 服务器路径 (backups): ${SERVER_IP}:${SERVER_BAK_DIR}/${BACKUP_DIR_NAME}.tar.gz${NC}"
        echo -e "${GREEN}📍 服务器路径 (bak): ${SERVER_IP}:/var/www/ai-assistant/bak/${BACKUP_DIR_NAME}.tar.gz${NC}"

        # 删除上传用的临时压缩包
        rm -f "${BACKUP_DIR_NAME}.tar.gz"

        # 检查云端备份数量
        CLOUD_COUNT=$(ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "ls -1 ${SERVER_BAK_DIR}/backup-*.tar.gz 2>/dev/null | wc -l" | tr -d ' ')
        BAK_COUNT=$(ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} "ls -1 /var/www/ai-assistant/bak/backup-*.tar.gz 2>/dev/null | wc -l" | tr -d ' ')
        echo -e "${GREEN}☁️  backups 目录备份数量: ${CLOUD_COUNT} 个${NC}"
        echo -e "${GREEN}☁️  bak 目录备份数量: ${BAK_COUNT} 个${NC}"
    else
...
echo "  用户上传文件: $((${AVATARS_COUNT:-0} + ${IMAGES_COUNT:-0} + ${FILES_COUNT:-0})) 个"
echo "  数据库: ${DB_SIZE}"
echo ""
echo -e "${CYAN}🔧 快速操作${NC}"
