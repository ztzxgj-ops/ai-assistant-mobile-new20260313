    ...
    def execute(self, sql, params=None):
        ...
        with self.get_cursor() as cursor:
            try:
                ...
                # 对于INSERT返回lastrowid，对于UPDATE/DELETE返回rowcount
...
class WorkPlanManagerMySQL:
    ...
    def add_plan(self, title, description='', deadline=None, priority='medium', status='pending', user_id=None):
        ...
        try:
            ...
            # ✨ 创建Mac桌面便签
            try:
                from sticky_note_manager import get_sticky_note_manager
                sticky_manager = get_sticky_note_manager(self.db)

                # 获取刚插入的任务ID
                task_id = self.db.connection.insert_id()

                # 创建便签
                sticky_manager.create_work_task_note(
                    task_id=task_id,
                    title=title,
                    content=description,
                    deadline=deadline,
    ...
    def update_plan(self, plan_id, user_id=None, **kwargs):
        ...
        # ✨ 如果状态更新为completed，删除对应的便签
        if 'status' in kwargs and kwargs['status'] == 'completed':
            try:
                from sticky_note_manager import get_sticky_note_manager
                sticky_manager = get_sticky_note_manager(self.db)
                sticky_manager.delete_work_task_note(task_id=plan_id)
            except Exception as e:
                print(f"⚠️ 便签删除失败: {e}")
